...
phone <- read.csv("../data/phone_soda2.csv", header=F)
colnames(phone) <- c("ts", "x", "y", "z")
glass <- read.csv("../data/glass_soda2.csv", header=F)
colnames(glass) <- c("ts", "x", "y", "z")
pebble <- read.csv("../data/pebble_soda2.csv", header=F)
colnames(pebble) <- c("ts", "x", "y", "z")

event_time <- c(21, 85, 137, 179, 227, 282, 290, 328)

phone_time_split = start_time + 21 + event_time
phone$ts2 <- phone$ts - min(phone$ts, na.rm=T)
phone$posixlt <- start_time + phone$ts2/1E9
phone$label = "unknown"
phone[phone$posixlt > phone_time_split[1] & phone$posixlt < phone_time_split[2], 'label'] = "walking"
phone[phone$posixlt > phone_time_split[2] & phone$posixlt < phone_time_split[3], 'label'] = "upstairs"
phone[phone$posixlt > phone_time_split[3] & phone$posixlt < phone_time_split[4], 'label'] = "standing"
phone[phone$posixlt > phone_time_split[4] & phone$posixlt < phone_time_split[5], 'label'] = "downstairs"
phone[phone$posixlt > phone_time_split[5] & phone$posixlt < phone_time_split[6], 'label'] = "standing"
phone[phone$posixlt > phone_time_split[6] & phone$posixlt < phone_time_split[7], 'label'] = "walking"
phone[phone$posixlt > phone_time_split[7] & phone$posixlt < phone_time_split[8], 'label'] = "running"
...
glass_time_split = start_time + 5.5 + event_time
...
## pebble
pebble_time_split = start_time - 52.5 + event_time
